...
fordFulkerson(Graph) ->
	...
	InitGraphAndCount = step1(Graph, 0),
	step2(element(1, InitGraphAndCount), element(2, InitGraphAndCount)).
...
step1(Graph, Counter) -> 
	...
	{graph_adt:setValV(lists:nth(2, lists:nth(1, SourceVertex)), marked, {"undefined", "100000"}, initialisierung(Graph, [])), Counter + 2}.
...
step2(Graph, Counter) ->
	...
	if
		 AllMarkedVerticesInspected == true -> step4(Graph, Counter); % Springe zu Schritt 4
			   						   true -> % Schritt 2B - Wähle eine beliebig markierte, aber noch NICHT inspizierte Ecke vi und
											   ...
											   if
													TargetMarked =/= {"nil", "nil"} -> step3(NewMarkedGraph1, NewCounter2 + length(element(1, NewMarkedGraph1)) + 1); % gehe zu Schritt 3
						  											 		   true -> step2(NewMarkedGraph1, NewCounter2 + length(element(1, NewMarkedGraph1)) + 1)  % gehe zu Schritt 2A
											   end
	end.
...
step3(Graph, Counter) ->
	...
	NewFlowAndCounter = setFlow(ReversedPath, Graph, NewCounter),
	...
	step2(ResetedGraph, NewCounter2).
...
setFlow([], Graph, Counter) ->
	{Graph, Counter};
setFlow([H|T], Graph, Counter) ->
	...
	if
		Predecessor == [] -> setFlow(T, Graph, Counter + 4);
					 true -> ForwardOrBackward = lists:nth(1, element(1, AttrMarkedValue)),
							 ...
							 if
		 						 ForwardOrBackward == Forward -> setFlow(T, graph_adt:setValE({IDPredecessor, ID}, maxis, {element(1, graph_adt:getValE({IDPredecessor, ID}, maxis, Graph)), integer_to_list((list_to_integer((element(2, graph_adt:getValE({IDPredecessor, ID}, maxis, Graph)))) + TargetFlowAsInt))}, Graph), Counter + 7);
								ForwardOrBackward == Backward -> setFlow(T, graph_adt:setValE({ID, IDPredecessor}, maxis, {element(1, graph_adt:getValE({ID, IDPredecessor}, maxis, Graph)), integer_to_list(list_to_integer(element(2, graph_adt:getValE({ID, IDPredecessor}, maxis, Graph))) - TargetFlowAsInt)}, Graph), Counter + 7);
