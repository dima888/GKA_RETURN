...
% Gibt alle Attribute und zugehˆrige Values in einer verschachtelten Liste zur¸ck, die
% in der ¸bergebenen Liste von Edges drin stecken 
...
% Gibt den Typ der Edges in der ¸bergebenen Liste zur¸ck (Liste muss aus gleich typigen Edges bestehen)
getEdgeType(EdgesInList) ->
	Edge = lists:nth(1, EdgesInList), 
	lists:nth(1, Edge).

% Gibt die passende Edge zu der ¸bergebenen ID aus einer Liste von Edges zur¸ck, Edgetyp = edgeD oder edgeU
...
%---------- HILFSMETHODEN -----------
% Gibt ein Boolean zur¸ck => True, wenn der ¸bergebene Val im graphen existiert, sonst false
...
% Gibt alle Attribute und zugehˆrige Values in einer verschachtelten Liste zur¸ck, die
% in der ¸bergebenen Liste von Vertices drin stecken
